...
function generateLeadsDataTable() {
    ...
    var table = $('#shawLeads').DataTable({
        ...
        "ajax": {
            "type": "POST",
            //"url": '/GetTableData',
            "url": '/GetShawLeadTableData',
            "data": function (d) {
                $.extend(d);
                d.leadfilter = $("#hdnLeadFilter").val();
        ...
        "fnInitComplete": function (oSettings, json) {
            ...
            var select = $('<select style="width:104px" class="pull-right" id="ddlLeadsFilter"><option value="0">All</option><option value="1">Shaw Leads</option><option value="2">Handed Off</option></select>')
                .on('change', function () {
                var val = $(this).val();
                $("#hdnLeadFilter").val($(this).val());
                table.column(0).search(val ? '^' + $(this).val() + '$' : val, true, false).draw();
            });
            $(select).insertBefore("#shawLeads_filter");
        }
    });
