...
links.Timeline.ItemBox.prototype.updateDOM = function () {
    ...
    if (divBox) {
        ...
        if (this.IsCopied) {
            divDot.className = "timeline-event timeline-event-dot-copied ui-widget ui-state-default";
        }
        else if (this.IsCopied==false) {
            ...
            if (this.groups.hasOwnProperty(groupName)) {
                ...
                while (i < iMax) {
                    ...
                    if (neighbors > maxItems) {
                        ...
                        var content = '<div title="' + title + '" data-toggle="tooltip" data-placement="top" title="" data-original-title="' + title + '">' + count + '</div>';
                        ...
                        else if (classNames.indexOf('form') > -1) {
                            clusterClassName = 'form';
                            selectedItemIndex = classNames.indexOf('form');
                        }
                        else if (classNames.indexOf('form-copied') > -1) {
                            clusterClassName = 'form-copied';
                            selectedItemIndex = classNames.indexOf('form-copied');
                        }
                        else if (classNames.indexOf('pageVisit') > -1) {
                            clusterClassName = 'pageVisit';
                            selectedItemIndex = classNames.indexOf('pageVisit');
                        }
                        else if (classNames.indexOf('pageVisit-copied') > -1) {
                            clusterClassName = 'pageVisit-copied';
                            selectedItemIndex = classNames.indexOf('pageVisit-copied');
                        }
                        else if (classNames.indexOf('email') > -1) {
                            clusterClassName = 'email';
                            selectedItemIndex = classNames.indexOf('email');
                        }
                        else if (classNames.indexOf('email-copied') > -1) {
                            clusterClassName = 'email-copied';
                            selectedItemIndex = classNames.indexOf('email-copied');
                        }
                        else if (classNames.indexOf('opportunity') > -1) {
                            clusterClassName = 'opportunity';
                            selectedItemIndex = classNames.indexOf('opportunity');
                        }
                        else if (classNames.indexOf('opportunity-copied') > -1) {
                            clusterClassName = 'opportunity-copied';
                            selectedItemIndex = classNames.indexOf('opportunity-copied');
                        }

                        var eventId = clusterItems[selectedItemIndex].event_id;
                        var IsCopied = clusterItems[selectedItemIndex].IsCopied;
...
links.Timeline.StepDate.prototype.next = function () {
    ...
    if (this.current.getMonth() < 6) {
        switch (this.scale) {
            // Commented by Rohit for hide the h/m/s stuff  for revert only remove group comments
