...
{
    public class ShawLeadsController : MainLayoutControllerBase
    {
        private readonly IShawLeadsViewModelAdapter _viewModelAdapter;
        private readonly ICRMLeadService _crmLeadService;
        private readonly ICRMCampaignService _crmCampaignService;
        private readonly IAccountService _accountService;

        public ShawLeadsController(IShawLeadsViewModelAdapter viewModelAdapter, ICRMLeadService crmLeadService, ICRMCampaignService crmCampaignService, IAccountService accountService)
        {
        ...
        public ActionResult ViewPage(ShawLeadsWebQuery query)
        {
            ...
            // Adapt data...
            ShawLeadsViewModel model = _viewModelAdapter.BuildShawLeadsViewModel(leads, accountObjectIDs, startDate, endDate);
            ...
            return View(ViewKind.ShawLeads, model, query);
        }

        [HttpPost]
        [Route("GetShawLeadTableData")]
        public ActionResult GetShawLeadTableData(int draw, int start, int length)
        {
            try
            {
                ...
                string leadfilter = Request.Form["leadfilter"];
                if (!String.IsNullOrEmpty(leadfilter) && Convert.ToInt16(leadfilter) > 0)
                {
                    if (leadfilter == "1")
                    {
                        // Show Shaw records
                        memberCol = memberCol.Where(p => string.IsNullOrEmpty(p.CopiedToDealerAccount) && p.AccountObjectID == User.MxtrAccountObjectID);
                    }
                    else if (leadfilter == "2")
                    {
                        // Show handed off records
                        memberCol = memberCol.Where(p => !string.IsNullOrEmpty(p.CopiedToDealerAccount));
                    }
                    else
                    {
                        //all
                ...
                IEnumerable<Models.Leads.ViewData.ShawLeadViewData> result = null;
        ...
        private static IEnumerable<Models.Leads.ViewData.ShawLeadViewData> AssignShawLeadViewData(IEnumerable<CRMCampaignDataModel> campaigns, IEnumerable<CRMLeadDataModel> displayedMembers)
        {
            IEnumerable<Models.Leads.ViewData.ShawLeadViewData> result;
            result = displayedMembers.Select(l => new Models.Leads.ViewData.ShawLeadViewData
            {
                ObjectID = l.ObjectID,
                ...
                CopiedToDealerAccount = l.CopiedToDealerAccount,
        ...
        private static IEnumerable<CRMLeadDataModel> SortDataColumns(IEnumerable<CRMLeadDataModel> filteredMembers, int sortColumn, string sortDirection)
        {
            switch (sortColumn)
            {
                ...
                case 8:
                    if (sortDirection == "asc")
                        filteredMembers = filteredMembers.OrderBy(x => x.CopiedToDealerAccount);
                    else
                        filteredMembers = filteredMembers.OrderByDescending(x => x.CopiedToDealerAccount);
                    break;
                default:
                    if (sortDirection == "asc")
        ...
        public class DataTableData
        {
            ...
            public List<Models.Leads.ViewData.ShawLeadViewData> data { get; set; }
        }
    }
}
